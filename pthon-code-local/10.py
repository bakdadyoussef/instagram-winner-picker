...
class GiveawayApp:
    def __init__(self, root):
        self.root = root
        root.title("IG Giveaway Picker - FINAL v4 (comment retry)")
        ...
        self.log.insert(tk.END, "🚀 FINAL v4: Comments fetch retry added.\n")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if '429' in str(e) or 'Too Many Requests' in str(e):
                    ...
                    return
                elif 'ConnectionException' in str(type(e)) or 'fail' in str(e):
                    self.log_msg(f"⚠️ Connection error (possible soft block). Retry {attempt+1}/{max_retries} after {retry_delay}s...")
                    time.sleep(retry_delay)
                    continue
                else:
    ...
    def _do_harvest(self):
        ...
        # ---- COMMENTS WITH RETRY ----
        self.log_msg("💬 Fetching comments (with retry on 'fail')...")
        commenters = set()
        total_comments = 0
        max_attempts = 3
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if 'fail' in str(e) or 'something went wrong' in str(e):
                    self.log_msg(f"⚠️ Comments fetch attempt {attempt} failed: {e}")
                    if attempt < max_attempts:
                        wait = 5 * (2 ** (attempt - 1))
                        self.log_msg(f"🔄 Retrying comments in {wait}s...")
                        time.sleep(wait)
                        continue
                    else:
                        self.log_msg("❌ Comments fetch failed after 3 attempts. Skipping comments.")
                        break
                else:
                    raise
            except Exception as e:
                self.log_msg(f"❌ Unexpected error fetching comments: {e}")
                raise
