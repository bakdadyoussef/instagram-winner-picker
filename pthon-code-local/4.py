...
from tkinter import scrolledtext, messagebox, ttk, Scale
import threading
import time
import secrets
import os
...
USER_AGENTS = [
    ...
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/119.0',  # fallback
]
FOLLOWER_CACHE_TTL = 3600  # seconds (1 hour)
...
class GiveawayApp:
    def __init__(self, root):
        self.root = root
        root.title("IG Giveaway Picker - Edge Navigator (429-proof)")
        root.geometry("850x780")
        root.resizable(False, False)

        # ---- STATUS FRAME ----
        self.status_frame = tk.LabelFrame(root, text="Connection Status", padx=10, pady=10)
        self.status_frame.grid(row=0, column=0, columnspan=4, sticky='we', padx=10, pady=5)
        ...
        tk.Label(root, text="Instagram Username (optional):").grid(row=1, column=0, sticky='e', padx=5, pady=5)
        self.username_entry = tk.Entry(root, width=30)
        self.username_entry.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(root, text="Password (optional):").grid(row=2, column=0, sticky='e', padx=5, pady=5)
        ...
        self.url_entry.grid(row=3, column=1, columnspan=3, padx=5, pady=5)
        ...
        # ---- DELAY SLIDER ----
        tk.Label(root, text="Request delay (seconds):").grid(row=6, column=2, sticky='e', padx=5, pady=5)
        self.delay_slider = Scale(root, from_=1, to=10, orient='horizontal', length=150)
        self.delay_slider.set(3)
        self.delay_slider.grid(row=6, column=3, sticky='w', padx=5, pady=5)
        ...
        self.progress.grid(row=8, column=0, columnspan=4, pady=10)

        # ---- LOG ----
        self.log = scrolledtext.ScrolledText(root, width=95, height=22, state='normal')
        self.log.grid(row=9, column=0, columnspan=4, padx=10, pady=10)
        self.log.insert(tk.END, "🚀 Ready. Click 'Connect via Edge' to log in and save cookies.\n")
    ...
    # ------------------------------------------------------------
    #  EDGE NAVIGATOR (unchanged, but we add user-agent randomisation)
    # ------------------------------------------------------------
    ...
    def browser_connect_thread(self):
        ...
        try:
            ...
            # Random user-agent for the browser itself
    ...
    # ------------------------------------------------------------
    #  LOAD COOKIES INTO INSTALOADER (with proxy support)
    # ------------------------------------------------------------
    def load_cookies_into_loader(self, loader):
        ...
        try:
            ...
            # Verify login
            test = instaloader.Profile.from_username(loader.context, self.username_entry.get().strip() or 'instagram')
            self.log_msg("✅ Cookies injected successfully.")
            return True
        except Exception as e:
            self.log_msg(f"⚠️ Cookie injection failed: {e}")
            return False
    ...
    # ------------------------------------------------------------
    #  MAIN HARVEST (with caching and delays)
    # ------------------------------------------------------------
    ...
    def harvest_thread_func(self):
        # We'll use a retry loop with backoff if we hit a 429
        max_retries = 5
        ...
        for attempt in range(max_retries):
            try:
                ...
                if '429' in str(e) or 'Too Many Requests' in str(e):
                    self.log_msg(f"🚨 429 detected. Retry {attempt+1}/{max_retries} after {retry_delay}s...")
                ...
                raise
        self.log_msg("❌ All retries failed. Aborting harvest.")
    ...
    def _do_harvest(self):
        ...
        # ---- INSTALOADER with global delay ----
        delay = self.delay_slider.get()
        ua = random.choice(USER_AGENTS)
        self.loader = instaloader.Instaloader(
            user_agent=ua,
            request_delay=delay  # <--- GLOBAL THROTTLE
        )
        # Optional proxy: uncomment and fill
        ...
        if not cookie_loaded:
            ...
            if os.path.exists(session_file):
                try:
                    ...
                    self.log_msg("✅ Session file loaded.")
                    cookie_loaded = True
                except:
                    pass
        if not cookie_loaded and username and password:
            self.log_msg("🔐 Trying normal login (risky)...")
            try:
                self.loader.login(username, password)
                self.loader.save_session_to_file(session_file)
                self.log_msg("✅ Login successful.")
                cookie_loaded = True
            except instaloader.exceptions.CheckpointRequiredException as e:
                match = re.search(r'(https?://[^\s]+)', str(e))
                if match:
                    self.log_msg(f"🚨 Checkpoint: {match.group(0)}")
                    self.log_msg("Complete it and click 'I've completed'.")
        ...
        if not cookie_loaded:
            self.log_msg("❌ No auth. Use Edge connect.")
            self.root.after(0, lambda: self.start_btn.config(state='normal'))
            return
        ...
        # ---- 1. COMMENTS (with random sleep after every 50) ----
        ...
        for comment in self.post.get_comments():
            ...
            if total_comments % 50 == 0:
                time.sleep(random.uniform(0.5, 1.5))  # extra jitter
        ...
        # ---- 3. FOLLOWERS (with caching) ----
        followers = set()
        if self.follow_var.get() == 1:
            self.log_msg(f"👥 Fetching followers of @{owner.username}...")
            # Try cache
            cache_file = f"followers_cache_{owner.username}.json"
                ...
                try:
                    for follower in owner.get_followers():
                        ...
                        if len(followers) % 200 == 0:
                            time.sleep(random.uniform(1, 3))  # heavy delay
                    ...
                    self.log_msg(f"⚠️ Followers fetch failed: {e}")
                    followers = set()
        else:
            self.log_msg("⏭️ Skipping followers.")
