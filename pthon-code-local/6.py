...
from tkinter import scrolledtext, messagebox, ttk, Scale, Checkbutton, IntVar, Entry
import threading
import time
import secrets
import os
...
class GiveawayApp:
    def __init__(self, root):
        self.root = root
        root.title("IG Giveaway Picker - Ultimate 429-proof")
        ...
        # ---- SKIP FOLLOWERS? (additional safety) ----
        ...
        self.delay_slider.set(5)  # higher default
        self.delay_slider.grid(row=7, column=3, sticky='w', padx=5, pady=5)

        # ---- PROXY ENTRY ----
        tk.Label(root, text="Proxy (optional, e.g. socks5://user:pass@ip:port):").grid(row=8, column=0, sticky='e', padx=5, pady=5)
        ...
        self.log.insert(tk.END, "🚀 Ultimate 429-proof mode. Use a proxy if possible.\n")
    ...
    # ------------------------------------------------------------
    #  LOAD COOKIES (NO EXTRA PROFILE FETCH!)
    # ------------------------------------------------------------
    def load_cookies_into_loader(self, loader):
        ...
        try:
            ...
            # We DO NOT verify by fetching a profile – that triggers 429.
            # We'll assume cookies are valid; we'll catch errors later.
            self.log_msg("✅ Cookies loaded (no verification request).")
            return True
        except Exception as e:
            self.log_msg(f"⚠️ Cookie injection error: {e}")
            return False

    # ------------------------------------------------------------
    #  RETRY CHECKPOINT (manual)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    #  MAIN HARVEST (with aggressive cooldown)
    # ------------------------------------------------------------
    ...
    def harvest_thread_func(self):
        # Retry with long cooldown for 429
        max_retries = 5
        retry_delay = 600  # 10 minutes – respect Instagram's suggested cooldown
        for attempt in range(max_retries):
            try:
                ...
                if '429' in str(e) or 'Too Many Requests' in str(e):
                    ...
                    # If we have a proxy, we could rotate it here
                    time.sleep(retry_delay)
                    # After first retry, maybe try a different proxy if available
                ...
                raise
        self.log_msg("❌ All retries failed. Give up or change IP/proxy.")
    ...
    def _do_harvest(self):
        ...
        # ---- INSTALOADER with global delay and proxy ----
        ...
        self.loader.request_delay = delay   # throttle all requests

        # Proxy setup
        proxy_str = self.proxy_entry.get().strip()
        if proxy_str:
            # Expect format: socks5://user:pass@ip:port or http://...
            proxies = {
                'http': proxy_str,
                'https': proxy_str
            }
            ...
            self.log_msg("ℹ️ No proxy – using direct IP (may be rate-limited).")
        ...
        # ---- 3. FOLLOWERS (skip if disabled) ----
        followers = set()
        skip_followers = (self.skip_followers_var.get() == 1)
        if self.follow_var.get() == 1 and not skip_followers:
            self.log_msg(f"👥 Fetching followers of @{owner_username} (this will take time)...")
            # Now we actually need the profile – do this only once
            try:
                profile = instaloader.Profile.from_username(self.loader.context, owner_username)
            except Exception as e:
                self.log_msg(f"⚠️ Failed to get profile: {e}. Follow filter disabled.")
                profile = None
            if profile:
