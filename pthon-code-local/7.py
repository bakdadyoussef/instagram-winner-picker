...
class GiveawayApp:
    def __init__(self, root):
        self.root = root
        root.title("IG Giveaway Picker - FINAL 429-proof")
        ...
        # ---- CREDENTIALS (username is now REQUIRED for context) ----
        ...
        self.log.insert(tk.END, "🚀 FINAL version – username is required for cookie login.\n")
    ...
    # ------------------------------------------------------------
    #  LOAD COOKIES – FIXED WITH VERIFICATION
    # ------------------------------------------------------------
    def load_cookies_into_loader(self, loader, username):
        ...
        try:
            ...
            # Build a dictionary of name->value for update
            cookie_dict = {}
            for cookie in cookies:
                # Only keep the name and value, we set domain/path globally
                cookie_dict[cookie['name']] = cookie['value']
            
            # Update session cookies
            loader.context._session.cookies.update(cookie_dict)
            
            # Set the X-CSRFToken header if we have it
            if 'csrftoken' in cookie_dict:
                loader.context._session.headers.update({'X-CSRFToken': cookie_dict['csrftoken']})
            
            # Set the username explicitly – this is crucial
            loader.context.username = username
            
            # Also set a consistent user-agent on the session
            loader.context._session.headers.update({'User-Agent': loader.user_agent})
            
            # Verify the session with a lightweight test
            if not loader.test_login():  # returns bool, may raise on 429
                self.log_msg("⚠️ Cookie session is not valid (test_login failed).")
                return False
    ...
    def harvest_thread_func(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise
        self.log_msg("❌ All retries failed. Change IP/proxy or wait longer.")
    ...
    def _do_harvest(self):
        ...
        # ---- INSTALOADER with delay and proxy ----
        ...
        # ---- AUTHENTICATION: try cookies first ----
        cookie_loaded = False
        if os.path.exists(self.cookie_file):
            self.log_msg("🍪 Loading cookies with verification...")
            cookie_loaded = self.load_cookies_into_loader(self.loader, username)
            if cookie_loaded:
                self.log_msg("✅ Cookie session active.")
            else:
                self.log_msg("⚠️ Cookie session invalid. Trying fallback methods.")
        ...
        # ---- FINAL FALLBACK: normal login ----
        if not cookie_loaded and username and password:
            self.log_msg("🔐 Attempting normal login (may trigger checkpoint)...")
            try:
        ...
        if not cookie_loaded:
            self.log_msg("❌ No valid authentication. Use Edge connect or correct credentials.")
            self.root.after(0, lambda: self.start_btn.config(state='normal'))
            return
