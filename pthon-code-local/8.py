...
class GiveawayApp:
    def __init__(self, root):
        self.root = root
        root.title("IG Giveaway Picker - FINAL v2 (No Password Login)")
        ...
        # ---- CREDENTIALS ----
        # Username is still required for context, but we NEVER use password.
        ...
        self.password_entry.insert(0, "LEAVE EMPTY")  # placeholder
        ...
        self.log.insert(tk.END, "🚀 FINAL v2: Password login is DISABLED. Use Edge to connect.\n")
    ...
    # ------------------------------------------------------------
    #  LOAD COOKIES – fixed verification
    # ------------------------------------------------------------
    def load_cookies_into_loader(self, loader, username):
        ...
        try:
            ...
            # Test login – this may raise 429, but we handle that separately
    ...
    def retry_after_checkpoint(self):
        ...
        # We simply restart the harvest; it will re-use cookies.
        self.start_harvest()

    # ------------------------------------------------------------
    #  MAIN HARVEST (without password login)
    # ------------------------------------------------------------
        ...
        for attempt in range(max_retries):
            try:
                ...
                if '429' in str(e) or 'Too Many Requests' in str(e):
                    ...
                    # This should never happen because we don't use password login,
                    # but if it does, we show the URL and enable the manual button.
                    match = re.search(r'(https?://[^\s]+)', str(e))
                    if match:
                        self.pending_checkpoint_url = match.group(0)
                        self.log_msg(f"🚨 CHECKPOINT REQUIRED! Open this URL:\n{self.pending_checkpoint_url}")
    ...
    def _do_harvest(self):
        ...
        if os.path.exists(self.cookie_file):
            ...
            if cookie_loaded:
                ...
                self.log_msg("⚠️ Cookie session invalid. Please re-connect via Edge.")
                self.root.after(0, lambda: self.start_btn.config(state='normal'))
                return
        else:
